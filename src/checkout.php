...
function autoship_checkout_create_order_line_item( WC_Order_Item_Product $item, $cart_item_key, $values, $order ) {
	...
	// and if not bail.
  $orders_created_via = apply_filters( 'autoship_order_created_via', [
    'checkout',
    'store-api'
  ] );
	if ( !in_array( $order->get_created_via(), $orders_created_via ) )
	return;
...
function autoship_create_scheduled_orders_on_checkout ( $order_id, $creation_date = null ) {
  ...
  if ( ( ! $order ) || !in_array( $created_via, $orders_created_via ) ) {
