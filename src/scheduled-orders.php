...
function autoship_get_scheduled_order_item_add_url( $args ){
  ...
  $params['redirect'] = urlencode( sanitize_text_field( $args['redirect'] ) );
...
function autoship_delete_scheduled_order_endpoint_wrapper (){
    ...
    // Add custom user security checks here if custom rights are added.
      ...
      __( sprintf( '<span class="action-request">Are you sure you want to Delete %s #%d?</span> <a class="confirm-action" href="%s">Confirm</a> or <a class="cancel-action" href="%s">Cancel</a>', $scheduled_order_label, $wp->query_vars['scheduled-order'], $url, $cancel ), 'autoship'), $scheduled_order_label, $url, $cancel, $wp->query_vars['scheduled-order'], $wp->query_vars['delete-scheduled-order-confirm'] );
...
function autoship_get_scheduled_order_link_params( $action ){
  ...
  $link['redirect']= isset( $_GET['redirect'] ) && !empty( $_GET['redirect'] )? sanitize_text_field( $_GET['redirect'] ) : NULL;
...
function autoship_scheduled_orders_v2_portal_display ( $customer_id, $autoship_customer_id, $template = 'scheduled-orders' ) {
  ...
  if ( empty( $autoship_orders ) || is_wp_error( $autoship_orders  ) ){
    ...
    $args = apply_filters( 'autoship_scheduled_orders_v2_portal_display_template_args', array(
      'customer_id'          => $customer_id,
      'autoship_customer_id' => $autoship_customer_id,
      'site_id'              => $client->get_site_id(),
      'token'                => $accessToken->tokenBearerAuth
    ) );
...
function autoship_scheduled_order_upsell_products_template_display ( $autoship_order, $customer_id, $autoship_customer_id ){
  ...
  if(in_array(strtolower($autoship_order['status']), [
    'processing',
    'locked'
  ]) || 'no' == autoship_get_scheduled_order_upsell_carousel() ) {
    return;
  }
