...
function autoship_get_ppcp_order_payment_data( $order_id, $order ) {
  ...
  if ( $wc_tokens ) {
    ...
    $order_id = $order->get_meta( \WooCommerce\PayPalCommerce\WcGateway\Gateway\PayPalGateway::ORDER_ID_META_KEY ) ?: wc_clean( wp_unslash( $_POST['paypal_order_id'] ?? '' ) );
...
function autoship_get_ppcp_cc_order_payment_data( $order_id, $wc_order ) {
  ...
  if( ($card_payment_token_id && $card_payment_token_id !== 'new') && $wc_tokens ) {
    ...
    // Get payment data from PayPal if not found in WP
    $order_id = $wc_order->get_meta( \WooCommerce\PayPalCommerce\WcGateway\Gateway\PayPalGateway::ORDER_ID_META_KEY ) ?: wc_clean( wp_unslash( $_POST['paypal_order_id'] ?? '' ) );
    $order_endpoint = \WooCommerce\PayPalCommerce\PPCP::container()->get('api.endpoint.order');
  ...
  if ( ! empty( $gateway_payment_token ) ) {
    if(!$token_description) {
      $token_description = $expiration ?
      sprintf( '%s ending in %s (expires %s)', ucfirst( strtolower( $brand ) ), $last_four, $expiration ) : sprintf( '%s ending in %s', ucfirst( strtolower( $brand ) ), $last_four );
    }
    $payment_data                       = new QPilotPaymentData();
...
function autoship_delete_paypal_v3_payment_method( $valid, $type, $token, $method ) {
  ...
  if ( 'PayPalV3' == $type ){
    ...
    return ( $method->gatewayCustomerId == $gateway_customer_id && $method->gatewayPaymentId == $token->get_token() );
...
function autoship_add_general_payment_method( $token , $return = false ){
  ...
  */
  ...
  $data = array(
    ...
    'lastFourDigits'    => $gateway === 'ppcp-gateway' ? null : $token->get_last4(),
		'gatewayCustomerId' => $wc_customer_id,
  	'gatewayPaymentId'  => $payment_method_id,
		'description'       => $description
  );

  // PayPayl Payments adjustements
  if ( $type === 'PayPalV3' ) {
    if ( $gateway === 'ppcp-credit-card-gateway' ) {
      $data['gatewayPaymentType'] = 26;
    }
    if ( $gateway === 'ppcp-gateway' ) {
